...
function initializeChart() 
{
    ...
    chart.addTimeSeries(lineRoll,      {strokeStyle: 'rgb(255, 0, 255)', 	lineWidth: 4 });
    chart.addTimeSeries(linePitch,     {strokeStyle: 'rgb(255, 191, 0)',  lineWidth: 4 });
    chart.addTimeSeries(lineMagX,      {strokeStyle: 'rgb(244, 41, 65)',   	lineWidth: 4 });
    chart.addTimeSeries(lineMagY,      {strokeStyle: 'rgb(0, 176, 24)', 	lineWidth: 4 });
    chart.addTimeSeries(lineMagZ,      {strokeStyle: 'rgb(60, 150, 250)', 	lineWidth: 4 });
    chart.addTimeSeries(lineNN,        {strokeStyle: 'rgb(57, 255, 20)',   	lineWidth: 5 });
}
...
function sensorDataHandler(){
    ...
    } else { app.dataPreProcessing = "delta"; }
    ...
    if( $("#dataPageButton").hasClass("is-active") ){
        ...
        if(app.trainFlag == false){ 
          ...
          //  app.magHistory.unshift(app.lastMagReading);
          //  if(app.magHistory.length > app.magHistoryMax) app.magHistory.pop(); 
            ...
            if(app.useMagnetometer && app.dataPreProcessing == "delta" && app.neuroHaveFlag) {
                values[0] = magDelta[0];
                values[1] = magDelta[1];
                values[2] = magDelta[2];
        ...
        if(app.neuroHaveFlag == false && app.trainFlag == false){ //oncle we have a nn model we only visualize that
        ...
        if( app.trainingDataTrue.length >= 100 && app.trainingDataFalse.length >= 100 && app.neuroHaveFlag != true){
            app.trainFlag = true;
        }
        ...
        if(app.neuroHaveFlag){
            ...
            if (app.trainFlag) {
                ...
                if(app.dataPreProcessing == "delta"){
                    ...
                    var trainFalseAv = []; var falseAvTempX = []; var falseAvTempY = []; var falseAvTempZ = [];
             ...
             //timeout allows load spinner to work
                setTimeout(function(){ 
...
console.log("app.betweenDetectionsFlag: " + app.betweenDetectionsFlag + "\t" + "app.betweenDetectionsFalseIntervalFlag: " + app.betweenDetectionsFalseIntervalFlag);
        if(app.betweenDetectionsFlag == false && app.betweenDetectionsFalseIntervalFlag == true){
            ...
            if( $( ".audio-checkbox-label" ).hasClass( "is-checked" ) ){
                ...
                audioAlert.play();
                setTimeout(function(){ audioAlert.pause(); audioAlert = $("#audio-alert")[0]; }, 1500);
            } else {
        ...
        //    clearTimeout(alertTimeout);
            ...
            if(app.useMagnetometer && app.dataPreProcessing == "delta"){ app.betweenDetectionsFlag
                setTimeout(function(){ 
                    app.betweenDetectionsFlag = false; 
                }, 5000);
...
app.onMagnetTestButton = function() {
    ...
    var magnetTestInterval = setInterval(() => { 
       ...
       if(netFieldChange > 10){
            $("#magnetTestButton").css({"background-color":"#00b300"}); //green
       } else {
            $("#magnetTestButton").css({"background-color":"#b30000"}); //red
       }
    }, 200);
    ...
    setTimeout(() => {  
        ...
        $("#magnetTestButton").css({"background-color":"#51a8f9"}); 
        ...
        magWatchID = cordova.plugins.magnetometer.watchReadings(
            ...
            if(app.betweenDetectionsFlag && app.useMagnetometer && app.dataPreProcessing == "delta"){
                ...
                if(app.magHistory.length > app.magHistoryMax){
                    ...
                    //remove oldest data from history
                    app.magHistory.pop();


                    //don't update during touch detection event b/c target shouldn't become baseline
                //    if(app.betweenDetectionsFlag == false || app.dataPreProcessing == "smooth"){

                        var magAvTempX = []; var magAvTempY = []; var magAvTempZ = [];
                 ...
                 /*   } else {
                        //without using history during detection event
                        magNormal[0] = (Math.min(Math.max( ((magNormal[0] * 360 - 180) + app.lastMagReading.x) / 2, -179), 179) + 180) / 360;
                        magNormal[1] = (Math.min(Math.max( ((magNormal[1] * 360 - 180) + app.lastMagReading.y) / 2, -179), 179) + 180) / 360;
                        magNormal[2] = (Math.min(Math.max( ((magNormal[2] * 360 - 180) + app.lastMagReading.z) / 2, -179), 179) + 180) / 360;
                    } */
